    ...
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-3 lg:px-8">
        <Link to="/" className="flex items-center gap-2.5">
          <img src={logo} alt="Paramgold Agritech" className="h-10 w-10 object-contain" width={40} height={40} />
          <div className="leading-tight">
            <div className="text-base font-bold tracking-tight text-primary-deep">Paramgold</div>
            <div className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Agritech Pvt. Ltd.</div>
          </div>
        </Link>
            ...
            return (
              <Link
                ...
                className={`relative rounded-full px-3.5 py-2 text-sm font-medium transition-colors ${
                  active ? "text-primary-deep" : "text-foreground/75 hover:text-primary-deep"
                }`}
              >
                {n.label}
                {active && (
